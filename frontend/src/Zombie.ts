...
export default class Zombie extends Sprite {
    draw(s:any, time:number=Date.now(), noDebug:boolean = DEBUG) {
        ...
        this.data.x = (this.data.x + (timeDelta * this.data.velX)) * TILE_SIZE
        this.data.y = (this.data.y + (timeDelta * this.data.velY)) * TILE_SIZE
        s.push()
        s.translate(this.data.x ,this.data.y)
